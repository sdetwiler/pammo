...
class PropToolPanel(wx.Panel):
    ...
    def doPropsOverlap(self, p1, p2):
        pos1 = p1.getPos()
        scale1 = p1.getScale()
        size1 = (p1.getProp().getBitmap().GetWidth()*scale1/2., p1.getProp().getBitmap().GetHeight()*scale1/2.)

        pos2 = p2.getPos()
        scale2 = p2.getScale()
        size2 = (p2.getProp().getBitmap().GetWidth()*scale2/2., p2.getProp().getBitmap().GetHeight()*scale2/2.)
    ...
    def onMapDraw(self, display, gc, rect):
        ...
        if self.outlineButton.GetValue():
            ...
            for e in self.editor.getMap().getEntities():
                pos = e.getPos()
                scale = e.getScale()
                size = (e.getProp().getBitmap().GetWidth(), e.getProp().getBitmap().GetHeight())
        ...
        if self.tracking:
            ...
            size = (self.tracking.getProp().getBitmap().GetWidth(), self.tracking.getProp().getBitmap().GetHeight())
            left = pos[0] - size[0]/2*scale
            right = pos[0] + size[0]/2*scale
            top = pos[1] - size[1]/2*scale
            bottom = pos[1] + size[1]/2*scale
